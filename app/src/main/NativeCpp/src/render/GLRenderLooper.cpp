...
void GLRenderLooper::OnSurfaceCreated() {
    ...
    if (m_GLEnv == nullptr) {
        LOGCATE("GLRenderLooper::OnSurfaceCreated m_GLEnv is null");
        return;
    }

    m_EglCore = new EglCore(m_GLEnv->sharedCtx, FLAG_RECORDABLE);
    if (m_EglCore == nullptr) {
        LOGCATE("GLRenderLooper::OnSurfaceCreated failed to create EglCore");
        return;
    }
    ...
    if (m_OffscreenSurface == nullptr) {
        LOGCATE("GLRenderLooper::OnSurfaceCreated failed to create OffscreenSurface");
        delete m_EglCore;
        m_EglCore = nullptr;
        return;
    }
...
void GLRenderLooper::OnDrawFrame() {
    ...
    if (m_GLEnv == nullptr || m_OffscreenSurface == nullptr) {
        LOGCATE("GLRenderLooper::OnDrawFrame m_GLEnv or m_OffscreenSurface is null");
        return;
    }
...
void GLRenderLooper::OnSurfaceDestroyed() {
    ...
    if (m_GLEnv != nullptr && m_GLEnv->renderDone != nullptr) {
        m_GLEnv->renderDone(m_GLEnv->callbackCtx, m_FboTextureId);
    }
...
bool GLRenderLooper::CreateFrameBufferObj() {
    if (m_GLEnv == nullptr) {
        LOGCATE("GLRenderLooper::CreateFrameBufferObj m_GLEnv is null");
        return false;
    }
